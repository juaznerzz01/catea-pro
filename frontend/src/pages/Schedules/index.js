...
const Schedules = () => {
  ...
  useEffect(() => {
    async function loadTz() {
      try {
        const tzData = await getCompanyTimezone(user.companyId);
        setCompanyTimezone(tzData.timezone);
      } catch {
        setCompanyTimezone("America/Sao_Paulo");
      }
    }
    loadTz();
  }, [user.companyId]);

  // Converter UTC para Date no timezone da empresa
  const utcToCompanyDate = (utcDateStr) => {
    const tz = companyTimezone || "America/Sao_Paulo";
    const m = moment.utc(utcDateStr).tz(tz);
    // Criar Date com os componentes do timezone da empresa
      ...
      <Paper className={classes.mainPaper} variant="outlined" onScroll={handleScroll}>
        <Calendar
          ...
          events={schedules.map((schedule) => ({
            ...
            start: utcToCompanyDate(schedule.sendAt),
            end: utcToCompanyDate(schedule.sendAt),
          }))}
          startAccessor="start"
          endAccessor="end"
          style={{ height: 500 }}
