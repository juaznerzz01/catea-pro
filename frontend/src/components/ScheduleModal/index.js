...
const ScheduleModal = ({ open, onClose, scheduleId, contactId, cleanContact, reload }) => {
	...
	// Carregar timezone da empresa e ajustar sendAt inicial
	useEffect(() => {
		let interval;
		const loadTimezone = async () => {
			try {
				...
				// Ajustar sendAt padrão para o timezone da empresa
				if (!scheduleId) {
					const nowInTz = moment().tz(tz).add(1, 'hour').format('YYYY-MM-DDTHH:mm');
					setSchedule(prev => ({ ...prev, sendAt: nowInTz }));
				}
	...
	}, [open, companyId, getCompanyTimezone, scheduleId]);
