...
const Routes = () => {
  ...
  // Rotas 100% publicas - fora de qualquer provider de autenticacao
  const publicPaths = ["/politicas", "/termos", "/exclusao-dados"];
  ...
  if (isPublicPage) {
    return (
      <BrowserRouter>
        <Switch>
          <PublicRoute exact path="/politicas" component={PrivacyPolicy} />
          <PublicRoute exact path="/termos" component={TermsOfUse} />
          <PublicRoute exact path="/exclusao-dados" component={DataDeletion} />
        </Switch>
      </BrowserRouter>
    );
  }
          ...
          <Switch>
            ...
            <WhatsAppsProvider>
              <LoggedInLayout>
                ...
                <Route exact path="/google-maps-scraper" component={GoogleMapsScraper} isPrivate />
                {showCampaigns && (
                  <>
