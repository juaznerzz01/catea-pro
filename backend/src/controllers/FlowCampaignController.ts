...
export const updateFlowCampaign = async (
  ...
  const { companyId } = req.user;
  const { flowId, name, phrase, id, status, matchType, whatsappId } = req.body;

  const flow = await UpdateFlowCampaignService({ companyId, name, flowId, phrase, id, status, matchType, whatsappId });
