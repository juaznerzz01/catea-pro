...
export const store = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (!companyUser) {
    ...
    // Verificar se mensagem de boas-vindas está habilitada (company 1 = admin do sistema)
    const adminSettings = await CompaniesSettings.findOne({ where: { companyId: 1 }, attributes: ["sendWelcomeMessage", "welcomeMessageTemplate"] });
    const welcomeEnabled = adminSettings?.getDataValue("sendWelcomeMessage") === "enabled";

    if (welcomeEnabled) {
      const lang = (language || "pt").split("-")[0];
      const trialDate = dateToClient(date);
      const customTemplate = adminSettings?.getDataValue("welcomeMessageTemplate");
      ...
      if (customTemplate && customTemplate.trim()) {
        // Template personalizado - substituir variáveis
        whatsappBody = customTemplate
          .replace(/\{name\}/g, name)
        ...
        emailBody = whatsappBody.replace(/\n/g, "<br>");
      } else {
        // Template padrão multilíngue
        const defaults = {
          pt: {
            subject: `Bem-vindo à ${companyName}!`,
            body: `Olá ${name}, bem-vindo(a) à ${companyName}! 🎉\n\nSeus dados de cadastro:\n\n👤 Nome: ${name}\n🏢 Empresa: ${companyName}\n📧 Email: ${email}\n📱 Telefone: ${phone}\n📅 Trial válido até: ${trialDate}`
          },
          en: {
            subject: `Welcome to ${companyName}!`,
            body: `Hello ${name}, welcome to ${companyName}! 🎉\n\nYour account details:\n\n👤 Name: ${name}\n🏢 Company: ${companyName}\n📧 Email: ${email}\n📱 Phone: ${phone}\n📅 Trial valid until: ${trialDate}`
          },
          es: {
            subject: `Bienvenido a ${companyName}!`,
            body: `Hola ${name}, bienvenido(a) a ${companyName}! 🎉\n\nDatos de su cuenta:\n\n👤 Nombre: ${name}\n🏢 Empresa: ${companyName}\n📧 Email: ${email}\n📱 Teléfono: ${phone}\n📅 Prueba válida hasta: ${trialDate}`
          }
        };
        const tpl = defaults[lang] || defaults.pt;
        whatsappBody = tpl.body;
        emailSubject = tpl.subject;
        emailBody = tpl.body.replace(/\n/g, "<br>");
      }

      try {
        await SendMail({ to: email, subject: emailSubject, text: emailBody });
      } catch (error) {
      ...
      try {
        ...
        if (
          ...
          const wbotId = whatsappCompany.whatsapps[0].id;
          const wbot = getWbot(wbotId);
          const cleanPhone = phone.replace(/\D/g, "");
          await wbot.sendMessage(`${cleanPhone}@s.whatsapp.net`, { text: whatsappBody });
        }
      } catch (error) {
    ...
    try {
      ...
      return res.status(200).json({ token, refreshToken, user: serializedUser });
    } catch (loginErr) {
      // Se login automático falhar, retorna sucesso sem token
