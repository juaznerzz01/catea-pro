...
export const storeFacebook = async (
  ...
  try {
    ...
    // Buscar App ID e Secret do banco (CompaniesSettings) com fallback pro .env
    const companySettings = await CompaniesSettings.findOne({ where: { companyId } });
    const fbAppId = companySettings?.facebookAppId || process.env.FACEBOOK_APP_ID;
    const fbAppSecret = companySettings?.facebookAppSecret || process.env.FACEBOOK_APP_SECRET;

    const { data } = await getPageProfile(facebookUserId, facebookUserToken, fbAppId, fbAppSecret);

    console.log("[Facebook] storeFacebook - pages encontradas:", data?.length, "addInstagram:", addInstagram);

    if (!data || data.length === 0) {
      return res.status(400).json({
        error: "Nenhuma Página do Facebook encontrada. Você precisa ter uma Página (não perfil pessoal) vinculada à sua conta."
      });
    }
    const io = getIO();
    ...
    for await (const page of data) {
      ...
      const acessTokenPage = await getAccessTokenFromPage(access_token, fbAppId, fbAppSecret);

      // Sempre criar conexão Facebook para a página
      pages.push({
        companyId,
        name,
      ...
      // Se tem Instagram vinculado e pediu Instagram, criar conexão Instagram também
    ...
    console.error("[Facebook] storeFacebook ERRO:", errMsg, error?.response?.data || "");
    return res.status(400).json({
      error: `Erro ao conectar Facebook: ${errMsg}`
    });
  }
};
