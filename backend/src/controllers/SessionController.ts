...
export const forgotPassword = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    await SendMail({
      ...
      html: `<p>Clique no link abaixo para redefinir sua senha:</p><p><a href="${resetUrl}">${resetUrl}</a></p><p>Este link expira em 30 minutos.</p>`
    }, user.companyId);
    console.log('Password reset email sent to:', email);
  } catch (error) {
    console.error('Failed to send password reset email:', error);
    throw new AppError("Erro ao enviar e-mail de redefinição. Verifique as configurações SMTP.", 500);
  }
