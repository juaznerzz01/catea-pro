...
export const importXls = async (req: Request, res: Response): Promise<Response> => {
  ...
  const { number, name, email, validateContact, tags, followUp, products, productName, productStatus } = req.body;
  ...
  // Importar produtos - suporta campos separados (productName + productStatus)
  // ou formato legado combinado (products: "Produto1:status,Produto2:status")
  if (productName && typeof productName === "string" && productName.trim()) {
    try {
      await ContactProduct.create({
        ...
        status: (productStatus && typeof productStatus === "string" && productStatus.trim()) || "pending"
      });
    } catch (error) {
      logger.info("Erro ao criar Produto", error);
    }
  } else if (products && typeof products === "string" && products.trim()) {
    const productList = products.split(',').map(p => p.trim());
    for (const productEntry of productList) {
      try {
        const [pName, pStatus] = productEntry.split(':').map(s => s.trim());
        if (pName) {
          await ContactProduct.create({
            contactId: contact.id,
            companyId,
...
export const index = async (req: Request, res: Response): Promise<Response> => {
  const { searchParam, pageNumber, contactTag: tagIdsStringified, isGroup, source, onlyAgenda, columnFilters: columnFiltersStr } = req.query as IndexQuery;
  ...
  if (columnFiltersStr) {
    try {
      parsedColumnFilters = JSON.parse(columnFiltersStr);
    } catch (e) {
      parsedColumnFilters = undefined;
    }
  }
...
export const filterOptions = async (req: Request, res: Response): Promise<Response> => {
  ...
  // Buscar valores distintos para cada coluna filtrável
  const [names, numbers, emails, followUps, tags, products, statuses, attendants] = await Promise.all([
    Contact.findAll({
      where: { companyId },
    ...
    Contact.findAll({
      where: { companyId },
      attributes: [[fn("DISTINCT", col("number")), "number"]],
      order: [["number", "ASC"]],
      raw: true
    }),
    Contact.findAll({
      where: { companyId, email: { [Op.ne]: null, [Op.ne]: "" } },
      attributes: [[fn("DISTINCT", col("email")), "email"]],
      order: [["email", "ASC"]],
      raw: true
    }),
    Contact.findAll({
      where: { companyId, followUp: { [Op.ne]: null, [Op.ne]: "" } },
      attributes: [[fn("DISTINCT", col("followUp")), "followUp"]],
      order: [["followUp", "ASC"]],
      raw: true
    }),
    Tag.findAll({
  ...
  return res.json({
    name: names.map((n: any) => n.name).filter(Boolean),
    number: numbers.map((n: any) => n.number).filter(Boolean),
    email: emails.map((e: any) => e.email).filter(Boolean),
    followUp: followUps.map((f: any) => f.followUp).filter(Boolean),
    tags: tags.map((t: any) => t.name).filter(Boolean),
    products: products.map((p: any) => p.name).filter(Boolean),
    payment: statuses.map((s: any) => s.status).filter(Boolean),
    attendant: attendants.map((u: any) => u.name).filter(Boolean)
  });
};
...
export const update = async (
  ...
  if (contactData.number && oldContact.number != contactData.number && oldContact.channel == "whatsapp") {
...
export const getContactProfileURL = async (req: Request, res: Response) => {
  ...
  export const updateContactLanguage = async (
    req: Request,
    res: Response
  ): Promise<Response> => {
    const { contactId } = req.params;
    ...
    await contact.update({ language: language || null });
