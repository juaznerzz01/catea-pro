...
const server = app.listen(PORT, HOST, async () => {
  ...
  // Iniciar filas Bull IMEDIATAMENTE (não depende das sessões WhatsApp)
  await startQueueProcess();
  BullQueue.process();

  // Iniciar sessões WhatsApp em paralelo (não bloqueia as filas)
  ...
  Promise.all(allPromises).catch(err => {
    logger.error(`Error starting WhatsApp sessions: ${err.message}`);
  });
