...
import { cancelFlowAITimeout } from "../../helpers/FlowAITimeoutHelper";
...
    // ── Tradução automática do caption da mídia ──
...
        }
      } catch (translateErr) {
        console.error("[verifyMediaMessage] Erro na tradução automática:", translateErr);
      }
    }
...
    await ticket.update({
      lastMessage: translatedBody || body || media.filename
    });
...
if (!fromMe && ticket && chatbotEnabled /* && isFirstContact */) {
  ...
  if (ticket.flowWebhook) {
    ...
    await cancelFlowAITimeout(ticket.id);
  ...
  if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
    if (queueIntegration?.urlN8N) {
      ...
      const payload = {
        ...
        integration: {
          id: queueIntegration.id,
          name: queueIntegration.name,
          type: queueIntegration.type
        },
        apiToken: whatsapp?.token || null,
        apiUrl: process.env.BACKEND_URL || null
      };

      try {
        const response = await axios.post(queueIntegration.urlN8N, payload, {
        ...
        if (data && data.reply && typeof data.reply === "string" && data.reply.trim()) {
          ...
          console.log(`[n8n] Resposta automática enviada para ticket ${ticket.id}: "${data.reply.trim().substring(0, 50)}..."`);
        }
      } catch (error) {
        console.error(
          `[n8n] Erro ao chamar webhook ${queueIntegration.urlN8N} para ticket ${ticket.id}:`,
          error?.message || error
        );
      }
    }
    ...
    console.log("[FLOW-DEBUG] flowbuilder branch", { isMenu, ticketStatus: ticket.status, lastFlowId: ticket.lastFlowId, lastMessage: ticket.lastMessage });
    if (!isMenu) {
      const integrations = await ShowQueueIntegrationService(
      ...
      console.log("[FLOW-DEBUG] isMenu=true, calling flowBuilderQueue", { ticketStatus: ticket.status });
...
const flowBuilderQueue = async (
  ...
  // Check if current node is a menu - menus should be processed regardless of ticket status
  const currentNode = nodes.find(node => node.id === ticket.lastFlowId);
  const isMenuNode = currentNode?.type === "menu";
...
const handleMessage = async (
  ...
  try {
    ...
    if (flow) {
      ...
      isOpenai = lastNode?.type === "openai" || lastNode?.type === "gemini";
      isQuestion = lastNode?.type === "question";
    }
    ...
    if (isOpenai && !isNil(flow) && !msg.key.fromMe) {
      const nodeSelected = flow.flow["nodes"].find(
        (node: any) => node.id === ticket.lastFlowId
      );
    ...
    // ========================================================================
    // Flow AI Mode: IA assumiu via Saida IA do FlowBuilder
    // ========================================================================
    const flowAIMode = (ticket.dataWebhook as any)?.flowAIMode;
    if (flowAIMode && !msg.key.fromMe) {
      const flowAIApiKey = (ticket.dataWebhook as any)?.flowAIApiKey;
      const flowAIPrompt = (ticket.dataWebhook as any)?.flowAIPrompt || "";

      if (flowAIApiKey) {
        const aiSettings = {
          name: "FlowAI",
          prompt: flowAIPrompt || "Você é um assistente virtual inteligente. Continue o atendimento de forma natural e prestativa.",
          voice: "texto",
          voiceKey: "",
          voiceRegion: "",
          model: "gpt-4o-mini",
    ...
    // ========================================================================
    // Flow Builder: Menu - processar resposta do cliente quando está em um menu
    // ========================================================================
    if (!isNil(flow) && isMenu && !msg.key.fromMe) {
      console.log("[FLOW-MENU] Menu detected, calling flowBuilderQueue", { ticketId: ticket.id, lastFlowId: ticket.lastFlowId, status: ticket.status });
      await flowBuilderQueue(
        ticket,
        msg,
        wbot,
    ...
    console.log("[FLOW-DEBUG-GATE1]", { imported: ticket.imported, fromMe: msg.key.fromMe, isGroup: ticket.isGroup, queue: ticket.queue?.id || ticket.queueId, user: ticket.user?.id || ticket.userId, isBot: ticket.isBot, integrationId: whatsapp.integrationId, useIntegration: ticket.useIntegration, isMenu, flowStopped: ticket.flowStopped, lastFlowId: ticket.lastFlowId });
    if (
      !ticket.imported &&
      !msg.key.fromMe &&
      !ticket.isGroup &&
