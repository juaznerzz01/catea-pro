    ...
    try {
      ...
      try {
        ...
        if (enabled && contactNumber?.language) {
          ...
          // Separar assinatura ("> NomeUsuario\n") do corpo da mensagem
          // para traduzir apenas o conteúdo e preservar a assinatura intacta
          let signatureLine = "";
          let messageContent = formattedBody;
          const signatureMatch = formattedBody.match(/^(> .+\n)/);
          if (signatureMatch) {
            signatureLine = signatureMatch[1];
            messageContent = formattedBody.slice(signatureLine.length);
          }

          const translated = await translateText(messageContent, userLang, contactLang, ticket.companyId);
          if (translated) {
            textToSend = signatureLine + translated;
            translatedBody = signatureLine + translated;
