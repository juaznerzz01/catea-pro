...
const CreateCompanyService = async (
  ...
  // Tratar planId vazio/inválido
  const safePlanId = planId && !isNaN(Number(planId)) ? Number(planId) : 1;
  ...
  try {
    ...
    await t.rollback();
    // Tratar erros de constraint unique (email duplicado, etc.)
    if (error.name === "SequelizeUniqueConstraintError") {
      const field = error.errors?.[0]?.path || "unknown";
      if (field === "email") {
        throw new AppError("ERR_USER_EMAIL_ALREADY_EXISTS", 400);
      }
      throw new AppError("ERR_COMPANY_DUPLICATE_FIELD", 400);
    }
    throw new AppError(error.message || "ERR_COMPANY_CREATION_FAILED", 500);
  }
};
